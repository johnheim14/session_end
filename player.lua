...
function Player.addItem(id, name, description, weight, actionName, onUse)
    ...
    table.insert(Player.inventory, { id=id, name=name, description=description, weight=w, actionName=actionName, onUse=onUse })
...
function Player.removeItem(index)
    ...
    if item then Player.currentWeight = Player.currentWeight - item.weight; table.remove(Player.inventory, index) end
end
...
function Player.attemptMove(dx, dy)
    ...
    for _, obj in ipairs(allObjects) do
        if obj.isBlocking then GameLog.add("Blocked by " .. obj.name .. "."); return end
    end
