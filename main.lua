...
function love.draw()
    ...
    -- [UPDATED] Draw Map SEPARATELY (Let STI handle the camera for tiles)
    -- We pass the negative camera values to shift the map left/up
    ...
    if gameState == "LOOTING" then
        ...
        local drawX = (Player.gridX * Map.tileSize) + 20
        local drawY = (Player.gridY * Map.tileSize) - 20
        
        -- Background Box
        love.graphics.setColor(0, 0, 0, 0.9)
        love.graphics.rectangle("fill", drawX, drawY, 150, #lootList * 20 + 10)
        love.graphics.setColor(0, 1, 0) -- Green Border
        love.graphics.rectangle("line", drawX, drawY, 150, #lootList * 20 + 10)
        
        -- List Items
        for i, obj in ipairs(lootList) do
            if i == lootIndex then
                love.graphics.setColor(1, 1, 0) -- Yellow for selected
                love.graphics.print("> " .. obj.name, drawX + 5, drawY + 5 + ((i-1)*20))
            else
